...
type Game() = 
    inherit GameWindow(853, 480, GraphicsMode.Default, 
                       "F# SimpleGL Sample")

    let vs = VertexShader.Create "C:\Users\Maik Klein\Documents\Visual Studio 2012\Projects\SimpleGL\HelloCube/vs.glsl"
    let fs = FragmentShader.Create "C:\Users\Maik Klein\Documents\Visual Studio 2012\Projects\SimpleGL\HelloCube/fs.glsl"
    ...
    override o.OnRenderFrame(e) = 
       ...
       // let node1_1   = Node(Rotation(r),Some(mesh),[])
        ...
        let node1_1 = { trans = Rotation(r); mesh = Some(mesh); nodeList =[]}
        let node1 = { trans = RelRotation(t,r); mesh = None; nodeList =[node1_1]}
        let node2 = { trans = RelRotation(t4,r1); mesh = Some(mesh); nodeList =[]}
        let scene = { trans = Translate(t); mesh = Some(mesh); nodeList =[ node1;node2]}
